...
namespace CARBDS
{
    class Program
    {
        ...
        {
            Console.WriteLine("Car Blocking Game BDS Version :)");
            ...
            HashStatistic.RootHash = GetHash(Cars);
            HashStatistic.GoalHash = GetHash(Cars2);


            CarNode RootNode = new CarNode(Cars,null);
            GoalCarNode GoalNode = new GoalCarNode(Cars2, null);
            BFS BFS = new BFS();
            Timer.Start();
            //BFS.bfsroot(RootNode);
            //BFS.bfsgoal(GoalCarNode);
            Parallel.Invoke(() => BFS.bfsroot(RootNode),() => BFS.bfsgoal(GoalNode));
