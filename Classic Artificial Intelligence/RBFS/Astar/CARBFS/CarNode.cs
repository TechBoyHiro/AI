...
namespace RBFS
{
    ...
    public class CarNode
    {
        ...
        public List<CarNode> Successor()
        {
            ...
            AStarInfraStructure.Heuristic(Cars);
            // this -4 sometimes may work ......
            ///int MaxHardibility = Hardibility + (HeuristicInfrastructure.Cars_Number - AStarInfraStructure.UnSortedCar)  - 4;
            //int MaxHardibility = Movement + (AStarInfraStructure.UnSortedCar * 4) +1 ;
            foreach (Car item in Cars)
            {
                if (item.Direction == Direction.Horizontal)
                {
                    if (item.StartColumn + item.length <= 5) // check if it has the ability to goes right
                    {
                        for (int i = item.StartColumn + item.length; i <= 5; i++)
                        {
                            if (Board[item.StartRow, i] == 0) // checking the rightside movement 
                            {
                                ...
                                //{
                                    int hardibility = AStarInfraStructure.Heuristic(Copy);
                                    //if (hardibility <= MaxHardibility)
                                    //{
