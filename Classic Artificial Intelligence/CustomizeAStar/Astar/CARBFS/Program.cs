...
namespace CARAStar
{
    class Program
    {
        ...
        {
            Console.WriteLine("Car Blocking Game A* Version :)");
            ...
            CarNode RootNode = new CarNode(Cars,null,16);
            int MainCarColumn = 0;
            Timer.Start();
            while (MainCarColumn <= 4)
            {
                MainCarColumn = RootNode.Cars.Where(x => x.CarId == 1).First().StartColumn;
                RootNode.Move(1, 1, Side.Right);

            }
            //AStar AStar = new AStar();
            //AStar.Astar(RootNode);
        }
